    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try
        {
                if(pass != null)
                {
                    ...
                    PreparedStatement ps1=con.prepareStatement("update CustomerDetails set Name = ?,Password = ?,PhoneNumber = ?,Address = ?,IDProofNumber = ? where UserID = ?");
                    ps1.setString(1,name);
                    ps1.setString(2,pass);
                    ps1.setString(3,number);
                    ps1.setString(4,address);
                    ps1.setString(5,idproof);
                    ps1.setString(6,user);

                    ps1.executeUpdate();
                    session.setAttribute("check","1");
                    session.setAttribute("message", "Update Details Successfully");            
